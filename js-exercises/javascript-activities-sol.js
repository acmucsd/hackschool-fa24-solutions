...
function logTrip(tripName, tripLen, costPerDay){
    ...
    const totalCost = tripLen * costPerDay;
    
    // Log the trip name and the total cost
...
function cheaperTrip(tripObject1, tripObject2) {
  // Calculate individual costs for both trips
  const cost1 = tripObject1.tripLen * tripObject1.costPerDay / tripObject1.numTravellers;
  const cost2 = tripObject2.tripLen * tripObject2.costPerDay / tripObject2.numTravellers;
  
  // Compare the costs and return the cheaper trip
  return cost1 < cost2 ? tripObject1 : tripObject2;
}
...
/* MAKE YOUR FUNCTION HERE */
const isTripIncluded = (destinationArray, tripDestination) => {
  for (let i = 0; i < destinationArray.length; i++) {
      if (destinationArray[i] === tripDestination) {
          return true;
      }
  }
  return false;
...
// Async function to fetch and log the exchange rate
